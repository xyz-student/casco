...
?>
  ...
  <div>
	...
	<div class="banner">
		<img src="image/cooling-banner.jpg" style="width:100%;"/>
	</div>	
...
<div class="container">
	...
	<div class="row">
			<?
			foreach($products=sqlfetch("select * from category where actstat='1' order by fld_order ") as $product)
			{  ?>
				<div class="col-md-3 mycls">
				<div class="grow pic">
					<a href="<?=SITE_URL?>products/<?=urlencode($product['name'])?>.html"><img src="<?=SITE_URL?>uploaded_files/cat/<?=$product['photo']?>" class="img-responsive" /></a>
				</div>
				<h1 class="myh2"><?=$product['name']?></h1>
				
				<a href="<?=SITE_URL?>products/<?=urlencode($product['name'])?>.html" class="getbtn4 center-block">Read More</a>
